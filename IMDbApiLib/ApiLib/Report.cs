...
namespace IMDbApiLib
{
    public partial class ApiLib
    {
        ...
        public Task DownloadReportAsync(string id, string filePath, Language language = Language.en, string options = "")
        {
            try
            {
                ...
                // await _http.DownloadFileTaskAsync(url, filePath);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public Task<byte[]> ReportBytesAsync(string id, Language language = Language.en, string options = "")
        {
            try
            {
                if (string.IsNullOrEmpty(options))
                ...
                throw new NotImplementedException("Not ported for Blazor.");
                // var bytes = await _http.DownloadDataTaskAsync(url);
                // return bytes;
            }
            catch (Exception ex)
            {
        ...
        public async Task DownloadReportAsync(string id, string filePath, Language language, string options, ProgressData progress = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var bytes = await Utils.DownloadDataAsync(url, _http, progress, cancellationToken);
